<template>
  <div>
    ...
    <!-- Pagination -->
    <div v-if="posts.meta && ((posts.meta.links && posts.meta.links.length > 3) || posts.meta.last_page > 1)" class="mt-6">
      <div class="flex flex-wrap justify-center md:justify-between items-center gap-2">
        <!-- Handle standard Laravel pagination links structure -->
        <template v-if="posts.meta.links && posts.meta.links.length > 3">
          <div v-for="(link, i) in posts.meta.links" :key="i" class="px-1">
            <button v-if="link.url" 
                    @click="goToPage(link.url)" 
                    v-html="link.label" 
                    :class="{'px-4 py-2 bg-indigo-600 text-white rounded-md': link.active, 'px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-100': !link.active}"
        ...
        <!-- Handle simple pagination structure -->
        <template v-else-if="posts.meta.last_page > 1">
          <div class="flex space-x-2">
            <button 
              @click="goToPage(`${props.apiEndpoint}?page=1`)" 
              :disabled="posts.meta.current_page === 1"
              :class="{'opacity-50 cursor-not-allowed': posts.meta.current_page === 1}"
            ...
            <button 
              @click="goToPage(`${props.apiEndpoint}?page=${Math.max(1, posts.meta.current_page - 1)}`)" 
              :disabled="posts.meta.current_page === 1"
              :class="{'opacity-50 cursor-not-allowed': posts.meta.current_page === 1}"
              class="px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-100 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
              Previous
            ...
            <button 
              @click="goToPage(`${props.apiEndpoint}?page=${Math.min(posts.meta.last_page, posts.meta.current_page + 1)}`)" 
              :disabled="posts.meta.current_page === posts.meta.last_page"
              :class="{'opacity-50 cursor-not-allowed': posts.meta.current_page === posts.meta.last_page}"
              class="px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-100 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
              Next
            </button>
            
            <button 
              @click="goToPage(`${props.apiEndpoint}?page=${posts.meta.last_page}`)" 
              :disabled="posts.meta.current_page === posts.meta.last_page"
              :class="{'opacity-50 cursor-not-allowed': posts.meta.current_page === posts.meta.last_page}"
              class="px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-100 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
              Last
      ...
      <div class="mt-3 text-center text-sm text-gray-600">
        Showing {{ posts.meta.from || ((posts.meta.current_page - 1) * posts.meta.per_page + 1) }} 
        to {{ posts.meta.to || Math.min(posts.meta.current_page * posts.meta.per_page, posts.meta.total) }} 
        of {{ posts.meta.total }} posts
      </div>
    </div>
  </div>
</template>
...
const fetchPosts = async (url = props.apiEndpoint) => {
  loading.value = true;
  error.value = null;
...
const deletePost = async (slug) => {
  ...
  try {
    ...
    const currentPage = posts.value.meta?.links?.find(link => link.active)?.url || props.apiEndpoint;
